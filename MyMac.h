...
class MyMacWaitTimer;
class MyMacSendTimer;
class MyMacRecvTimer;
...
private:
	...
	Handler * 	txHandler_;
	MyMacWaitTimer	*waitTimer;
	MyMacSendTimer *sendTimer;
	MyMacRecvTimer *recvTimer;
	int repeatTx_;
	double cbrInterval_;
...
class MyMacWaitTimer: public MyMacTimer {
public: MyMacWaitTimer(MyMac *m) : MyMacTimer(m) {}
	void handle(Event *e);
};

//  Timer to use for finishing sending of packets
class MyMacSendTimer: public MyMacTimer {
public:
	MyMacSendTimer(MyMac *m) : MyMacTimer(m) {}
	void handle(Event *e);
};

// Timer to use for finishing reception of packets
class MyMacRecvTimer: public MyMacTimer {
public:
	MyMacRecvTimer(MyMac *m) : MyMacTimer(m) {}
	void handle(Event *e);
};
