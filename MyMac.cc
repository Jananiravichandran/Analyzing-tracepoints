...
MyMac::MyMac() : Mac() {
	...
	tx_active_ = 0;
	waitTimer = new MyMacWaitTimer(this);
	sendTimer = new MyMacSendTimer(this);
	recvTimer = new MyMacRecvTimer(this);
	//bind("repeatTimes_", &repeatTimes_);
	...
	bind("cbrInterval_", &cbrInterval_);
}

// Added by Sushmita to support event tracing (singal@nunki.usc.edu)
int 
MyMac::command(int argc, const char*const* argv)
{
	if (argc == 3) {
...
void MyMac::send(Packet *p, Handler *h)
{
	...
	// Create an dynamic array for random times
	double* random_times;
	random_times = new double[repeatTx_];
	
	// Store random times in this array	
	int i=0;
	while(i<repeatTx_) {
		int num_generated = rand();
		double delay = (num_generated%100)/100.0*cbrInterval_;
		max = (delay > max) ? delay: max;
		random_times[i] = delay;
		i++;
	}
	
	i=0;
	while(i<repeatTx_) {
		if(max==random_times[i]) {
			i++;
			continue;
		}
		else {
			Scheduler::instance().schedule(this, (Event*)p->copy(), random_times[i]);
			i++;
		}
	}
...
void MyMacTimer::restart(double time)
{
	if (busy_)
		stop();
	start(time);
...
void MyMacWaitTimer::handle(Event *)
{
	busy_ = 0;
	stime = rtime = 0.0;
...
void MyMacSendTimer::handle(Event *)
{
	busy_ = 0;
	stime = rtime = 0.0;
...
void MyMacRecvTimer::handle(Event *)
{
	busy_ = 0;
	stime = rtime = 0.0;
